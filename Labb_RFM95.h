...
#define RF95_SYMB_TIMEOUT       0x64     /// default register value is 0x08
#define RF95_MAX_PAYLOAD_LENGTH 0xff    /// 0xff is default, the RFM95 accepted payload length -> all packages can be received
#define PAYLOAD_LENGTH           0x40   /// if the Gateway would send a package to the end nodes it would have the payload size of 0x40
#define FREQ_HOP_PERIOD          0x00   /// 0x00 means freq hopping is turned off
#define LNA_MAX_GAIN             0x23   /// LowNoiseAmplifier Gain if the gateway is not set in automatic gain controll
...
/// Register names (LoRa Mode, can be found in the data sheet of the RFM95 table 85)
